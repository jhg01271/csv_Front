...
class CamApp(MDApp):
    ...
    def detect_faces(self, *args):
        ...
        if not self.face_found and len(faces) > 0:
            ...
            if img_filename:
                ...
                self.cropped_img_filename = fetch_and_crop_latest_image(self.mem_id)
                if self.cropped_img_filename:
                    print(f"크롭된 얼굴 이미지 파일명: {self.cropped_img_filename}")
    ...
    def stop_camera(self, *args):
        ...
        print("카메라 인식 종료")
        self.switch_to_signup()  # signup 페이지로 전환

    # 얼굴을 인식하고 이미지 저장이 완료되면 signup 페이지로 전환하는 함수
